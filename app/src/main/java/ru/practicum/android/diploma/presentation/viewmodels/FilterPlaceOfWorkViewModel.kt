...
class FilterPlaceOfWorkViewModel(
                    ...
                    when (response) {
                        is ResponseData.Data -> {
                            response.value.forEach {
                                if (it.id == countryId) {
                                    if (isSaving) {
                                        setState(PlaceOfWorkScreenState.Saved(it.name))
                                    } else {
                                        setState(PlaceOfWorkScreenState.CountryName(it.name))
                                    }
                                }
                            }
                        }
    ...
    fun saveFields(countryName: String, regionId: String) {
        if (countryName.isEmpty()) {
            ...
            setState(PlaceOfWorkScreenState.Saved(countryName))
        }
    ...
    fun setCountryName(countryName: String) {
        if (countryName.isNotEmpty()) {
            setState(PlaceOfWorkScreenState.CountryName(countryName))
        } else {
